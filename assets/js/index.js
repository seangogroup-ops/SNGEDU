const toast = document.getElementById('toast');
    ...
    async function loadPromoBanner(){
        ...
        if (!banner) return;
        try{
            const { data, error } = await sb.from('site_settings').select('payload').eq('key', 'promo_banner').single();
    ...
    async function loadAnnouncementPopup(){
        ...
        if (!overlay) return;
        try{
            const { data, error } = await sb.from('site_settings').select('payload').eq('key', 'announcement_popup').single();
            const payload = (!error && data && data.payload) ? data.payload : null;
    ...
    async function refreshSessionAndRenderAccount(){
        ...
        // CHỈ tải lịch sử giao dịch (sepay_orders + balance_transactions + pro_grants) khi
        // đang thực sự xem sub-tab "Lịch sử giao dịch". Trước đây gọi vô điều kiện mỗi lần
        // mở tab Tài khoản, kể cả khi đang ở "Tổng quan" (sub-tab lịch sử đang ẩn) -> thừa
        // 1 lượt truy vấn mỗi lần vào tab Tài khoản, cộng dồn theo lượt truy cập rất nhanh.
        const activeSubtab = document.querySelector('.acct-subtab.active');
        if (activeSubtab && activeSubtab.dataset.subtab === 'history') loadAcctOrderHistory();
    }
